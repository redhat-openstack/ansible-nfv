...
def gen_stl(devices, traffic, duration, multiplier):
    ...
    try:
        ...
        for port in my_ports:
            streams = register().get_streams(devices[port]['src_mac'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_ports', action='store')
    ...
    if args.action == "parse":
        ...
        # send traffic in one queue per port
        # queue id will be different
        if args.test_ports:
            test_ports = json.loads(args.test_ports)
            used_queues = []
            for port in range(len(traffic)):
                for key, value in traffic[port]['queues'].items():
                    if key not in used_queues:
                        value['pps'] = test_ports[port]
                        if test_ports[port] != 0:
                            used_queues.append(key)
                        test_ports[port] = 0
                    else:
                        value['pps'] = 0
