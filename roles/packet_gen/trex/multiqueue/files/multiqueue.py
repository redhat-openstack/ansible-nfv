...
def parse_pmd_stats(queues_json, pmd_stats, pps):
    ...
    for port_index, port_value in enumerate(queues_json):
        for index, queue in port_value["queues"].items():
            queue["hyp_queues"] = []
            queue["hyp_queues"].\
                append(queue_ids[port_index][int(index)*2])
            queue["hyp_queues"].\
                append(queue_ids[port_index][int(index)*2 + 1])
            queue["rate"] = pps[port_index][index]
    return queues_json
