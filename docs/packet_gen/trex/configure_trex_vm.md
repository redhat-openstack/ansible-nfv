...
```
ansible-playbook playbooks/packet_gen/trex/configure_trex_vm.yml -e cloud_resources=create -e @/path/to/openstack_task_vars.yml
```
...
```
ansible-playbook playbooks/packet_gen/trex/configure_trex_vm.yml -e cloud_resources=external -e @/path/to/existing_resources.yml
```