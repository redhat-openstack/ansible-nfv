                    ...
                    if y:
                        ...
                        if int(value) in range(int(ips_limits[0]), int(ips_limits[1])):
                            if queue not in output_item['queues']:
                                output_item['queues'][queue] = {"pps": 0, "isg": 0, "ips": []}
    ...
    def get_streams(self, src_mac, dst_mac, qratio, multiplier, **kwargs):
        ...
        for queue in qratio.keys():
            if qratio[queue]["pps"] > 0:
                vm = STLScVmRaw([STLVmFlowVar(name="dst_ip", value_list=qratio[queue]["ips"], op="inc"),
                                 STLVmWrFlowVar(fv_name='dst_ip', pkt_offset='IP.dst'),
                                 STLVmFixIpv4(offset='IP'),
                                 STLVmFixChecksumHw(l3_offset="IP", l4_offset="UDP",
                                                    l4_type=CTRexVmInsFixHwCs.L4_TYPE_UDP )
                                ])
                streams.append(self.create_stream(float(qratio[queue]["pps"]), vm, src_mac, dst_mac, multiplier))
        return streams
...
if __name__ == '__main__':
    ...
    parser.add_argument("--action", required=True, choices=['learning', 'parse', 'gen_traffic'], action='store')
